...
class NST:
    ...
    def variational_cost(generated_image):
        ...
        parameters:
            generated_image [tf.Tensor of shape (nh, nw, 3)
                or shape (1, nh, nw, 3)]:
        ...
        height_diff = (generated_image[..., 1:, :, :] -
                       generated_image[..., :-1, :, :])
        width_diff = (generated_image[..., 1:, :] -
                      generated_image[..., :-1, :])
        return (tf.reduce_sum(tf.abs(height_diff)) +
                tf.reduce_sum(tf.abs(width_diff)))
